...
const DataContractCreateTransition = require(
  '@dashevo/dpp/lib/dataContract/stateTransition/DataContractCreateTransition',
);
...
describe('applyStateTransitionHandlerFactory', function main() {
  ...
  beforeEach(async () => {
    const services = await startDapi();
  ...
  it('should register new identity and store contract', async () => {
    ...
    try {
      ...
      const contract = await dapiClient.getDataContract(contractId);
