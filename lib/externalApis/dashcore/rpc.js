...
const getStatus = () => new Promise((resolve, reject) => {
  client.getnetworkinfo((err, networkResponse) => {
    if (err) {
      reject(new DashCoreRpcError(err.message));
    }
    const networkInfo = networkResponse.result;
    client.getblockchaininfo((blockchainInfoErr, blockchainResponse) => {
      if (blockchainInfoErr) {
        reject(new DashCoreRpcError(blockchainInfoErr.message));
      }
 ...
 * getStatus: (function(): Promise<any>),
