...
const BLOCK_QUERY = 'tm.event = \'NewBlock\'';
const events = {
  NEW_BLOCK: 'block',
};
...
class TransactionsClient extends EventEmitter {
  ...
  start() {
    ...
    this.wsClient.subscribe(BLOCK_QUERY);
    this.wsClient.on(BLOCK_QUERY, (message) => {
...
TransactionsClient.BLOCK_QUERY = BLOCK_QUERY;
TransactionsClient.events = events;
