...
class dapiList extends require('./mempoolBase') {
    constructor(port, pubkey, mynode, namespace = 'dapilist') {
        super(port)
        this.node = mynode;
        this.pubkey = pubkey;
        this.ready = false;
    ...
    init(key = 'nodelist') {
        ...
        this.dapilist.events.on('write', (dbname, hash, entry) => {
            ...
            //check if value includes this node. If not we don't pin it.
            const node = obj.find(o => o.payee === this.pubkey);
            if (node) {
                this.dapilist._ipfs.pin.add(hash);
                console.log("written and pinned dapilist with new entry: " + JSON.stringify(obj));
                console.log("now complete list is: " + JSON.stringify(this.dapilist.get(key)));
            }else{
                console.log('this node is not in entry. Add it!');
                obj.push(this.node);
                this.writeValue(obj);
            }
        })
    ...
    addNode(node) {
        ...
        if(nodelist.length > 0){
            ...
            const existing = nodelist.find(o => o.payee === this.pubkey);
            if (existing) {
    ...
    updateNodeTimestamp() {
        const nodelist = this.getValue();
        let node = nodelist.find(o => o.payee === this.pubkey);
        if (node){
