...
const getMasternodesList = (mode = 'status', filter = '') => new Promise((resolve, reject) => {
  client.masternodelist(mode, filter, (err, r) => {
    if (err) {
      reject(err);
    }
    resolve(r.result);
...
const getBlockHeaders = (hash, count = 2000, verbose = true) => new Promise((resolve, reject) => {
  client.getBlockHeaders(hash, count, verbose, (err, r) => {
    if (err) {
      reject(err);
    }
    resolve(r.result);
