...
const getAddressSummary = async (address, noTxList = false, from, to) => {
  const res = Array.isArray(address) ? await get(`/addrs/${address.join()}?noTxList=${+noTxList}&from=${from}&to=${to}`) : await get(`/addr/${address}?noTxList=${+noTxList}&from=${from}&to=${to}`);
  return res;
};
