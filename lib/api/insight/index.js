...
const getUTXO = async (address, from, to, fromHeight, toHeight) => {
  ...
  if (Array.isArray(address)) {
    res = await get(`/addrs/${address.join()}/utxopage?from=${from}&to=${to}&fromHeight=${fromHeight}&to=${toHeight}`);
    return res;
  }
  res = await get(`/addrs/${address}/utxopage?from=${from}&to=${to}&fromHeight=${fromHeight}&to=${toHeight}`);
  return res;
};
