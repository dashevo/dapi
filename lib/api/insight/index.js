...
const getTransactionsByAddress = async (address, from, to, fromHeight, toHeight) => {
  ...
  if (Array.isArray(address)) {
    res = await get(`/addrs/${address.join()}/txs?from=${from}&to=${to}&fromHeight=${fromHeight}&to=${toHeight}`);
    return res;
  }
  res = await get(`/addrs/${address}/txs?from=${from}&to=${to}&fromHeight=${fromHeight}&to=${toHeight}`);
  return res;
};
