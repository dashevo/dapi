...
const getBlocks = async (limit, blockDate) => {
  let queryParams = '';
  if (limit && blockDate) {
    queryParams = `?limit=${limit}&blockDate=${blockDate}`;
  } else if (blockDate) {
  ...
  // TODO: Should we really return just a subset of the response?
  return res.blocks;
};
...
// TODO: Rename this to something more canonical, e.g. getAddressSummary
const getAddressAddress = async (address) => {
...
const getTransactionsByAddress = async (address) => {
  const res = await get(`/addrs/${address}/txs`);
  return res;
};
...
const postTransactionSendix = async (rawtx) => {
