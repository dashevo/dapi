...
class Insight {
  ...
  // todo: remove, it's not real mn ips list
  getMnList() {
  ...
  async sendRawTransition(rawTransition) {
    throw this.post('/ts/send', { rawts: rawTransition });
  }
  ...
  async getBestBlockHeight() {
    const res = await this.get('/bestBlockHeight');
    return res.height;
  }

  async getBlockHash(blockHeight) {
    const res = await this.get(`/block-index/${blockHeight}`);
    return res.blockHash;
  }

  async getMasternodesList() {
    return this.get('/masternodes/list');
  }
}
