...
class DashDriveAdapter extends AbstractDashDriveAdapter {
   ...
   * @param {string} rawTransitionPacket - raw data packet serialized to hex string
   * @param {string} rawTransitionHeader - special transaction
   * @return {Promise<string>} - packet id
   */
  addSTPacket(rawTransitionPacket, rawTransitionHeader) {
    return this.request('addSTPacket', {
      stPacket: rawTransitionPacket,
      stateTransition: rawTransitionHeader,
    });
  }
