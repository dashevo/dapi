...
const InstantLocksCache = require('./InstantLocksCache');
...
function subscribeToNewTransactions(
  ...
  const instantLockCache = new InstantLocksCache();
  ...
  filterEmitter.on('match', (transaction) => {
    ...
    // Store transaction in a cache waiting to receive an instant lock
    instantLockCache.addTransactionHashToInstantLockWaitingList(transaction);
  });
  ...
  filterEmitter.on('block', (block) => {
    ...
    instantLockCache.incrementBlockSpentInCacheForEveryTransaction();
  });

  filterEmitter.on('instantLock', (instantLock) => {
    const isTransactionInWaitingList = instantLockCache.isInCache(instantLock.txid);

    if (isTransactionInWaitingList) {
      instantLockCache.removeTransactionHashFromWaitingList(instantLock.txid);
