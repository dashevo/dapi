...
function subscribeToNewTransactions(
  ...
  filterEmitter.on('instantLock', (instantLock) => {
    const isTransactionInWaitingList = transactionsAndBlocksCache.isInInstantLockCache(instantLock.txid);

    if (isTransactionInWaitingList) {
      transactionsAndBlocksCache.removeTransactionHashFromInstantSendLockWaitingList(instantLock.txid);
