...
function subscribeToNewTransactions(
  ...
  mediator.on(ProcessMediator.EVENTS.HISTORICAL_DATA_SENT, async () => {
    while (sendingCacheData) {
      const unsentTransactions = transactionsAndBlocksCache.getUnsentTransactions();
      unsentTransactions
        .forEach(tx => mediator.emit(ProcessMediator.EVENTS.TRANSACTION, tx));

      const unsentMerkleBlocks = transactionsAndBlocksCache.getUnsentMerkleBlocks();
      unsentMerkleBlocks
