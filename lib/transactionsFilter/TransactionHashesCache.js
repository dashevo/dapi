...
class TransactionHashesCache {
  ...
  addBlock(block) {
    ...
    const reverseTransactionHashes = blockTransactionHashes.map((hash) => {
      const hashBuffer = Buffer.from(hash, 'hex');
      const reversedBuffer = BufferUtils.reverse(hashBuffer);
      return reversedBuffer.toString('hex');
    });

    const merkleBlock = MerkleBlock.build(
      block.header,
      reverseTransactionHashes,
  ...
  getUnsentTransactions() {
    // TODO Rename unsent to something
