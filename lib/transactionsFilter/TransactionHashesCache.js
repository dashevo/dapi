class TransactionHashesCache {
  ...
  addBlock(block) {
    ...
    block.transactions
      .forEach(t => this.transactions[t.hash].linkedBlockHashes.push(blockHash));
  ...
  getMatchedTransactions(block) {
    const blockTxHahes = block.transactons.map(tx => tx.hash);
    return blockTxHahes
      .filter(txHash => this.hasTransactionHash(txHash)
        && this.transactions[txHash].linkedBlockHashes.length === 0)
