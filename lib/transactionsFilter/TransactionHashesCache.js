...
class TransactionHashesCache {
  ...
  addBlock(block) {
    ...
    const removeBefore = this.blocksProcessed - BLOCKS_TO_STAY_IN_INSTANT_LOCK_CACHE;
    for (const hash in this.transactionHashesMap) {
      if (hasOwnProperty.call(this.transactionHashesMap, hash)) {
        ...
        const needsToBeRemoved = isInCache && this.transactionHashesMap[hash] > removeBefore;
        if (needsToBeRemoved) {
