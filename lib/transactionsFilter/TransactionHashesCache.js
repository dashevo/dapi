...
class TransactionHashesCache {
  ...
  addBlock(block) {
    ...
    for (let hash in this.transactionHashesMap) {
      const isInCache = this.isInInstantLockCache(hash);
      const needsToBeRemoved = isInCache && this.transactionHashesMap[hash] + BLOCKS_TO_STAY_IN_INSTANT_LOCK_CACHE < this.blocksProcessed
      if (needsToBeRemoved) {
