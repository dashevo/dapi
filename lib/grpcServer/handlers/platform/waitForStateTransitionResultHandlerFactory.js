...
function waitForStateTransitionResultHandlerFactory(
  ...
  async function waitForStateTransitionResultHandler(call) {
    ...
    const query = `tm.event = 'Tx' AND tx.hash = '${Buffer.from(stateTransitionHash).toString('hex')}'`;

    const data = await new Promise((resolve) => {
      tenderDashWsClient.once(query, (message) => {
        resolve(message);
      });
    });
    ...
    if (result && result.code !== undefined && result.code !== 0) {
      ...
      error.setData(cbor.encode(abciError.data));
