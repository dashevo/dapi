...
function subscribeToTransactionsWithProofsHandlerFactory(
  ...
  async function subscribeToTransactionsWithProofsHandler(call, callback) {
    ...
    for await (const { merkleBlock, transactions } of merkleBlocksIterator) {
      const merkleBlockResponse = new TransactionsWithProofsResponse();
      merkleBlockResponse.setRawMerkleBlock(merkleBlock.toBuffer());

      await wrappedCall.write(merkleBlockResponse.toObject());

      const transactionResponse = new TransactionsWithProofsResponse();
      transactionResponse.setRawTransactions(
        transactions.map(tx => tx.toBuffer()),
      );

      await wrappedCall.write(merkleBlockResponse.toObject());
      cache.removeTransactions(transactions);
