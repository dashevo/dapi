const { MerkleBlock, BloomFilter, Transaction } = require('@dashevo/dashcore-lib');
...
async function subscribeToHistoricalTransactions(
  ...
  const rawMerkleBlocks = await coreAPI.getMerkleBlocks(filter, blockHash, count);
  ...
  const rawTransactionsGroupedByBlock = await Promise.all(transactionHashes
    .map(transactionHash => coreAPI.getRawTransaction(transactionHash)));
