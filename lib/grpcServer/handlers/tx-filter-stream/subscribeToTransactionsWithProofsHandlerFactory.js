...
function subscribeToTransactionsWithProofsHandlerFactory(
  ...
  async function subscribeToTransactionsWithProofsHandler(call, callback) {
    ...
    const wrappedCall = new AcknowledgingWritable(call);
    ...
    for await (const { merkleBlock, transactions } of historicalDataIterator) {
      await sendTransactionsResponse(wrappedCall, transactions);
      await sendMerkleBlockResponse(wrappedCall, merkleBlock);

      if (count === 0) {
        // remove sent transactions from cache
        mediator.emit(ProcessMediator.EVENTS.HISTORICAL_TRANSACTIONS_SENT, transactions);
      }
    ...
    if (count === 0) {
      ...
      mediator.on(
        ...
        tx => sendTransactionsResponse(wrappedCall, [tx]),
      );
      mediator.on(
        ProcessMediator.EVENTS.MERKLE_BLOCK,
        merkleBlock => sendMerkleBlockResponse(wrappedCall, merkleBlock),
      );
    }
