...
const quorumsd = require('quorums-dash')
		...
		return {
			post: {
				quorum: function(req, res, next) {
					...
					Promise.all([insight.getMnList(), insight.getLastBlockHash(), insight.getAddress(body.data.txId)])
						.then(([list, hash, addr]) => {
							let quorumData = {
								mnList: list,
								refHash: hash,
							...
							return quorumsd.validate(body.data, body.signature, quorumData, qTempPort) ////QDEVTEMP - remove qTempPort
						})
						.then(validQuorum => {
							if (!validQuorum) {
								console.log('Invalid quorum for this request')
								next();
							}
							else {
