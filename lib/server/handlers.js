...
const nodefeed = require('../mempool/dapiList');
		...
		return {
			...
			get: {
				...
				mnList: function(req, res) {
                    if (app.config.filterMNList){
                        let body;
                        insight.performGetRequestPlus('/masternodes/list', req, res, function (body) {
                            let json = JSON.parse(body);
                            console.log(json);
                            let dapilist = new nodefeed();
                            body = dapilist.getFilteredList(body);
                            res.send(body);
                        });
                    } else {
                        insight.performGETRequest('/masternodes/list', req, res)
                    }
                },
				auth: {
