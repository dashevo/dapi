...
function handleRequiredField(body, expectedFields, res) {
  let valid = true;
  if (expectedFields && expectedFields.constructor.name === 'Object') {
    for (let i = 0; i < Object.keys(expectedFields).length; i += 1) {
      const param = Object.keys(expectedFields)[i];
      const rules = expectedFields[param];

      if (!body[param] && rules.required !== false) {
...
class Handlers {
  constructor(app) {
    const { insight, authService, quorum } = app;
    ...
    return {
        ...
        quorum(req, res, next) {
          ...
          if (!handleRequiredField(body, {
            verb: {
              required: true,
              type: 'enum',
              value: ['add', 'commit', 'remove', 'state', 'listen', 'migrate', 'auth'],
            },
            qid: { required: true, type: 'number' },
            data: { required: true, type: 'json' },
          }, res)) {
            // If field doesn't meet required rules, will be returned false and enter here in order
            // to break continuation of the logic
            next();
          }
          ...
          quorum.isValidQuorum(body, qTempPort)
            .then((isValid) => {
              if (!isValid) {
                ...
                returnResponse(quorum.performAction(
                  body.verb,
                  { qid: body.qid, data: body.data },
                ), res);
              }
            });
        },
      ...
      get: {
        ...
        blockHash(req, res) {
          const { params: { hash } } = req.params.hash;
          insight.performGETRequest(`/block/${hash}`, req, res);
        },
        rawBlock(req, res) {
          const { body: { blockHash } } = req.body.blockHash;
        ...
        address: {
          get(req, res) {
            const { params: { addr } } = req.params.addr;
