...
function sendHeartBeat() {
    ...
    if (list) {
        ...
        let arraylist = JSON.parse(JSON.stringify(list));
        let i = 0
        arraylist.forEach(function(){
            if (arraylist[i].payee === self.config.pubKey){
                console.log('ts: ' + arraylist[i].ts);
                if (((new Date).getTime() -  arraylist[i].ts ) >= self.nodeListTimeStampTimeout) {
                    cl('node timed out! Remove from dapilist');
                    // log info about the dapi node to be removed
                    console.log("ts: " + arraylist[i].ts);
                    console.log("pubkey = [" + arraylist[i].payee + "]")
                    // remove this node because its timestamp hasn't been updated for too long and probably suffered a power outage
                    self.removeNodeFromDapiList(arraylist[i].payee);
                } else {
