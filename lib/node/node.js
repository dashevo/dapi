...
const nodeList = require('../mempool/dapiList'); //this keeps track of all enabled dapi nodes to filter MN list during development (EV-402)
...
class Node {
    constructor(params) {
        ...
        this.node = { payee: this.config.pubKey, ts: new Date().getTime(), ip: this.ip, version: this.version, insight: this.insight};
        this.socks = {};
        this.mempool = new kvs(_.get(params, 'mempoolPort'));
        this.dapilist = new nodeList(_.get(params, 'dapiListPort'), this.config.pubKey, this.node);
    ...
    start() {
        ...
        cl(`Started node`);
        cl(`Check if OrbitDB ready \n`);
        var _checkReady = setInterval(function() {
            if (self.dapilist.ready) {
                clearInterval(_checkReady);
                console.log('OrbitDB ready. Add node to dapilist!');
                self.addNodeToDapiList();
                console.log('Node added to dapilist. Start the HeartBeat!');
                pollFunc(sendHeartBeat.bind(self), self.nodeListHeartBeatTimeout, self.nodeListHeartBeatInterval);
            }else{
                console.log('waiting for OrbitDB...');
            }
        }, 2000);
    }
...
function sendHeartBeat() {
    let list = this.dapilist.getList();
    if (list) {
        // remove own node
        let self = this;
        ...
        if (!this.dapilist.updateNodeTimestamp()){
            console.log('node not in dapilist. Add it!');
            self.addNodeToDapiList();
        }
        ...
        list.forEach(function(){
            if (list[i].payee !== self.config.pubKey){
                ...
                if (((new Date).getTime() -  list[i].ts ) >= self.nodeListTimeStampTimeout) {
                    ...
                    // log info about the dapi node to be removed (just for debugging. To be removed)
                    console.log("ts: " + list[i].ts);
