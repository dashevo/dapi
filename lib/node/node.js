...
function sendHeartBeat() {
    ...
    if (list) {
        // remove node
        let self = this;
        //TODO; check if node is still in dapilist


        list.forEach((e) => {
            var pubkey = JSON.stringify(e.pubkey);//TODO: check if correct value
            var ts = JSON.stringify(e.ts);
            if (((new Date).getTime() - ts ) >= self.nodeListTimeStampTimeout) {
                ...
                this.removeNodeFromDapiList(pubkey);
            } else {
