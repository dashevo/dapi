...
class Node {
    constructor(params) {
        ...
        this.nodelisthash = '';
        this.nodeListHeartBeatTimeout = _.get(params, 'nlhbTimeout'); // if positiv value in milliseconds until polling dapilist stops; default -1 = no timeout
        this.nodeListHeartBeatInterval = _.get(params, 'nlhbInterval'); // interval in milliseconds; 15 min. = 900000 | set to a few seconds for testing
        this.nodeListTimeStampTimeout = _.get(params, 'nltsTimeout'); // interval in milliseconds; 30 min. = 1800001 (should be > 2x nodeListHeartBeatInterval to make sure we don't delete a connected node that just didn't refresh in between)
        this.init();
        setTimeout(function() {
            self.start();
    ...
    addNodeToDapiList(cb) {
        let node = { ip: this.ip, version: this.version, insight: this.insight, pubkey: this.config.pubKey, rep: this.config.rep, pub: this.config.pub, ts: new Date().getTime()};
        //node = { ip: line, payload: { pubkey: this.config.pubKey, rep: this.config.rep, pub: this.config.pub } }
