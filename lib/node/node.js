...
class Node {
    constructor(params) {
        ...
        this.nodeListHeartBeatInterval = 900000; // interval in milliseconds; 15 min. = 900000 | set to a few seconds for testing
        this.nodeListTimeStampTimeout = 1800001; // interval in milliseconds; 30 min. = 1800001 (should be > 2x nodeListHeartBeatInterval to make sure we don't delete a connected node that just didn't refresh in between)
        this.init();
    ...
    addNodeToDapiList(cb) {
        let node = { ip: this.config.ip, version: this.version, insight: this.insight, pubkey: this.config.pubKey, rep: this.config.rep, pub: this.config.pub, ts: new Date().getTime()};
