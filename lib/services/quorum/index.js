...
const quorumService = {
  ...
  async dkg(quorumNodes) {
    ...
    // needs to be saved in kv store
	  const kv = new KeyValueStore(config.node.ipfs);
    //kv.writeValue('16d5bd5088656193913414cc9141394bc38b02d717f0d2b55dcaeebe00adc8f3', 'Xc7HnuA5t9x5Gyvw5QfvKAcPpMDbjwtPch', verificationVector, 'vvec');


    // needs to be encrypted for every node and saved in kv store
    for (sk of secretKeyContribution) {
      logger.debug(sk.toString());
    }

    kv.writeValue('16d5bd5088656193913414cc9141394bc38b02d717f0d2b55dcaeebe00adc8f3', 'Xc7HnuA5t9x5Gyvw5QfvKAcPpMDbjwtPch', secretKeyContribution, 'skcontr');
  ...
  async getQuorumCalcHash() {
    return new Promise((resolve, reject) => {
      dc.getCurrentBlockHeight()
        ...
        .then((hash) => {
          dc.getBlockHeaders(hash, qDash.getPOWAveragingNum())
            .then((headers) => {
