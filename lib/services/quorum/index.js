...
const quorumService = {
  ...
  async getQuorumForUser(body) {
    return new Promise((resolve, reject) => {
        Promise.all([insight.getMasternodesList(), this.getQuorumCalculationHash()])
            .then(([list, hash]) => {
  ...
  async isValidQuorum(body, qTempPort) {
    return new Promise((resolve, reject) => {
        Promise.all([insight.getMasternodesList(), this.getQuorumCalculationHash(), dashcore.getAddress(body.data.txId)])
            .then(([list, hash, addr]) => {
                const quorumData = {
                    mnList: list,
                    calcHash: hash,
                    refAddr: addr,
                    vin: this.getVin(pubkey)
                };
