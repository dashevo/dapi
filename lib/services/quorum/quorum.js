...
class Quorum {
  ...
  init() {
      ...
      this.getQuorumHashOnStartup()
          .then((hash) => {
              this.blockhash = hash;
              console.log('%s %s', this.blockhash, this.height );
              ...
              subscriber.on('message', (topic, msg) => {
                  ...
                  dashcore.getCurrentBlockHeight()
                      .then((height) => {
                          ...
                          console.log('%s %s %s %s %s', topic, msg, this.getQuorumHash(), height, this.is_handover);
                      })
              })
          })
  }
  ...
  //original getQuorumHash(). Now only used on DAPI startup to determine quorum before the first block comes in
  getQuorumHashOnStartup() {
