...
class Quorum {
      ...
      subscriber.on('message', (topic, msg) => {
          ...
          dashcore.getCurrentBlockHeight()
              .then((height) => {
                  ...
                  let is_heartbeat = height % heartbeat_interval === 0;
                  if (is_heartbeat) {
  ...
  joinQuorum() {
      ...
      //TODO: fix quorum-dash. Add method for identifying this dapi node's quorum
      this.getQuorum()
          .then((quorumData) => {
  ...
  getQuorum() {
      return new Promise((resolve, reject) => {
          Promise.all([dashcore.getMasternodesList(), this.getQuorumHash(), this.getVin(this.pubkey)])
              .then(([list, hash, vin]) => {
                  resolve(qDash.getQuorum(list, hash, vin));
              });
      });
  }
  ...
  getQuorumForUser(body) {
    return new Promise((resolve, reject) => {
        Promise.all([dashcore.getMasternodesList(), this.getQuorumHash()])
            .then(([list, hash]) => {
                resolve(qDash.getQuorum(list, hash, qDash.getVin(list, body.data.txId)));
            });
    });
  }
  ...
  isValidQuorum(body, qTempPort) {
    return new Promise((resolve, reject) => {
      Promise.all([dashcore.getMasternodesList(), this.getQuorumHash(), dashcore.getAddress(body.data.txId)])
        .then(([list, hash, addr]) => {
          const quorumData = {
            mnList: list,
            refHash: hash,
