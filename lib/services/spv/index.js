...
class spvService {
      ...
      message.inventory.forEach((m) => {
        ...
        switch (m.type) {
          case 1: // MSG_TX: if bloomfilter set this only happens on matched txs + false positives
            ...
            break;
          case 2: // MSG_BLOCK: occurst on every block even if bloomfilter is set
            this.peer.sendMessage(this.peer.messages.GetData.forBlock(cHash));
            break;
          case 3: // MSG_FILTERED_BLOCK: This never happens regardless of bloomfilter set or not
  ...
  // This is suppose to fetch merkle blocks but instead its fetching
  // unfiltered blocks
  getMerkleBlock(fromBlockHash) {
