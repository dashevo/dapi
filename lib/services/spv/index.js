...
const clearDisconnectedClientBloomFilters = ({ clients, currentTime }) => {
  if (!clients.length) {
    return [];
  }
  const hasDisconnectedThresholdInMsec = 60000;
  return clients.filter((client) => {
    if (currentTime - client.lastSeen.getTime() >= hasDisconnectedThresholdInMsec) {
      peer.sendMessage(client.peer.messages.FilterClear(client.filter));
      return false;
    }
    return true;
  });
...
class SpvService {
  constructor() {
    ...
    setInterval(() => {
      this.clients =
        clearDisconnectedClientBloomFilters({ clients: this.clients, currentTime: new Date() });
    }, 60000);
  }

  updateLastSeen(filter) {
    this.clients.find(client => client.filter === filter).lastSeen = new Date();
  }
