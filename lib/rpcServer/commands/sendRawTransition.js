...
const sendRawTransition = async (args, callback) => {
  try {
    ...
    if (transitionHeader.isUpdateTransition()) {
      ...
      dashDrive.pinPacket(transformedHeader, transitionPacket);
      // Todo: move it out of here in refactor branch
      dashDrive._commitPacket(transformedHeader, {});
    }
     ...
     */
    const transitionId = await coreAPI.sendRawTransition(transitionHeader.serialize());
    // }
