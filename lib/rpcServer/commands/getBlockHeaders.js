...
const getBlockHeadersFactory = (coreAPI) => {
   ...
   * @param {boolean} [args.verbose] - set true to get headers in json
   * @return {Promise<Array<Object>>}
   */
  async function getBlockHeaders(args) {
    validator.validate(args);
    const { offset, limit, verbose } = args;
    return coreAPI.getBlockHeaders(typeof offset === 'number' ? await coreAPI.getBlockHash(offset) : offset, limit, verbose);
  }
 ...
 *                      type: boolean
 *                      default: false
 *                      description: set true to get headers in json
 */
